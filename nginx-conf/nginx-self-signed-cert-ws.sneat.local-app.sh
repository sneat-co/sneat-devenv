...
KEY="/etc/ssl/private/local-app.sneat.ws.key"
CERT="/etc/ssl/certs/local-app.sneat.ws.crt"
...
sudo openssl req \
  ...
  -subj "/C=IE/ST=Munster/L=Limerick/O=Sneat.co/CN=local-app.sneat.ws" \
  -config openssl-local-app.sneat.ws.cnf \
  -keyout $KEY \
  -out $CERT
...
# openssl x509 -in /etc/ssl/certs/local-app.sneat.ws.crt -text -noout

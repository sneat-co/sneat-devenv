...
KEY="/etc/ssl/private/local-api.sneat.ws.key"
CERT="/etc/ssl/certs/local-api.sneat.ws.crt"
...
sudo openssl req \
  ...
  -subj "/C=IE/ST=Munster/L=Limerick/O=Sneat.co/CN=local-api.sneat.ws" \
  -config openssl-local-api.sneat.ws.cnf \
  -keyout $KEY \
  -out $CERT
...
# openssl x509 -in /etc/ssl/certs/local-api.sneat.ws.crt -text -noout
